...
__declspec(dllexport) int main(int argc, char *argv[])
{
    ...
    // Data to write to the target process memory
    int newValue = 1234;
    SIZE_T bytesWritten;

    // Write data to the target process memory
    BOOL success = WriteProcessMemory("-1", 0x12345678, &newValue, sizeof(newValue), &bytesWritten);
    if (success && bytesWritten == sizeof(newValue)) {
        printf("Memory written successfully. Bytes written: %zu\n", bytesWritten);
    } else {
